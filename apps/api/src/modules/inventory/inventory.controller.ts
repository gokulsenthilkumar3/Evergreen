...
export class InventoryController {
  ...
  async getInventoryHistory(
    ...
    const metrics: any = await this.inventoryService.getDashboardMetrics(
      from,
      to,
    );
    ...
    const trendMap = new Map<
      string,
      { date: string; kg: number; outward: number }
    >();
    ...
    const inwardHistory = Array.from(trendMap.values()).sort(
    ...
    return {
      ...
      kpis: [
        ...
        },
        {
          ...
          subValue:
            metrics.totalYarn % 60 > 0
              ? `(Incomplete bag: ${(metrics.totalYarn % 60).toFixed(2)} kg)`
              : '',
          color: '#2e7d32',
