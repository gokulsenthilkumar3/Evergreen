...
export class BillingController {
  ...
  async createInvoice(@Body() invoice: any) {
    ...
    for (const item of items) {
      if (!item.yarnCount)
        throw new BadRequestException('Yarn count is required for all items');
      if (!item.weight || parseFloat(item.weight) <= 0)
        throw new BadRequestException(
          'Weight must be greater than 0 for all items',
        );
    ...
    if (existing) {
      throw new BadRequestException(
        `Invoice number ${invoiceData.invoiceNo} already exists`,
      );
    }
    ...
    return this.prisma.invoice.create({
      data: {
        ...
        issuerSignature: invoiceData.issuerSignature || null,
        customerSignature: invoiceData.customerSignature || null,
        authorizedSignatory: invoiceData.authorizedSignatory || null,
  ...
  async addPayment(@Param('id') id: string, @Body() body: any) {
    ...
    return {
      ...
      message:
        newStatus === 'PAID'
          ? 'Invoice fully paid!'
          : `Partial payment recorded. Remaining: ₹${(invoice.total - newAmountPaid).toFixed(2)}`,
    };
  }
  ...
  async deletePayment(@Param('id') id: string) {
    ...
    if (!invoice)
      throw new BadRequestException('Invoice not found for this payment');

    const newAmountPaid = Math.max(0, invoice.amountPaid - payment.amount);
    const newStatus =
      newAmountPaid <= 0.01
        ? 'UNPAID'
        : newAmountPaid >= invoice.total - 0.01
          ? 'PAID'
          : 'PARTIAL';
