...
export class DashboardController {
      ...
      kpis: [
        {
          ...
          value: hasProductionData
            ? `${periodProduction.toFixed(0)} kg`
            : 'No Data',
          subValue: hasProductionData
            ? `${metrics.yarnBags} bags + ${metrics.yarnLooseKg?.toFixed(1) || 0} kg loose`
            : '',
          color: '#2e7d32',
          trend: '',
          comparison: '',
        ...
        },
        {
          ...
          value: hasCostData
            ...
            : 'No Data',
          subValue:
            hasCostData && costPerKg > 0 ? `₹${costPerKg.toFixed(2)} / kg` : '',
          color: '#ed6c02',
          trend: '',
          comparison: '',
          hasData: hasCostData,
  ...
  async getCharts(@Query('from') from?: string, @Query('to') to?: string) {
    ...
    const startDate = from
      ? new Date(from)
      : new Date(endDate.getTime() - 13 * 24 * 60 * 60 * 1000);
    ...
    const prodMap = new Map<
      string,
      { produced: number; waste: number; consumed: number }
    >();
